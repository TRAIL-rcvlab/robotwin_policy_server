...
class TrainDP3Workspace:
    ...
    def run(self):
        ...
        # --- 👇 [修改开始]：实现夹爪动作的加权采样 ---
        # print("Checking for dataset oversampling weights...")
        
        # # 检查你的 dataset 类是否实现了 'chunk_weights'
        # if hasattr(dataset, 'chunk_weights'):
        #     print(f"Found 'chunk_weights'. Creating WeightedRandomSampler to fix mode collapse.")
            
        #     weights = dataset.chunk_weights
            
        #     # 创建采样器
        #     sampler = torch.utils.data.WeightedRandomSampler(
        #         weights=weights,
        #         num_samples=len(weights),
        #         replacement=True  # 允许重复采样，确保过渡数据被充分学习
        #     )
        ...
        #     # !! 关键 !!
        #     # 当使用自定义 sampler 时, 必须禁用 shuffle
        #     if 'shuffle' in dataloader_cfg:
        #         print("Disabling 'shuffle' in dataloader_cfg, as 'sampler' is provided.")
        #         dataloader_cfg['shuffle'] = False
            
        #     # 使用加权采样器创建 DataLoader
        #     train_dataloader = DataLoader(dataset, sampler=sampler, **dataloader_cfg)
            
        #     # 统计信息
        #     num_transitions = torch.sum(weights > 1.0).item()
        #     total_chunks = len(weights)
        #     print(f"Sampler active. Total chunks: {total_chunks}. Transition chunks (boosted): {num_transitions}")

        # else:
        #     print("WARNING: Dataset does not have 'chunk_weights' attribute.")
        #     print("         Using standard DataLoader. Gripper 'mode collapse' problem will likely persist.")
        #     train_dataloader = DataLoader(dataset, **cfg.dataloader)
    ...
    def get_policy_and_runner(self, cfg, usr_args):
        ...
        #             os.path.join(
        #                 DP3_ROOT,
        #                 f"{usr_args['ckpt_path']}/{usr_args['checkpoint_num']}.ckpt"
        #             ))
