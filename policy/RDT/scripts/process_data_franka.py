...
def load_hdf5(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root:
        right_gripper, right_arm = (
            root["/joint_action/gripper"][()],
            root["/joint_action/positions"][()],
        )
        image_dict = dict()
        image_dict["head_camera"] = root[f"/observation/front_camera/rgb"][()]
        # for cam_name in root[f"/observation/"].keys():
        #     image_dict[cam_name] = root[f"/observation/{cam_name}/rgb"][()]

    # return left_gripper, left_arm, right_gripper, right_arm, image_dict
    return right_gripper, right_arm, image_dict

def decode_jpeg_bytes(padded_bytes):
    """从 HDF5 读取的填充字节中解码 JPEG 图像"""
    # 转为 bytes 并去除右侧 \x00
    raw_bytes = padded_bytes.tobytes().rstrip(b"\x00")
...
def data_transform(path, episode_num, save_path):
    ...
    for i in range(episode_num):
        ...
        right_gripper_all, right_arm_all, image_dict = (load_hdf5(
            os.path.join(path, f"{i}.hdf5")))
        qpos = []
        actions = []
        cam_high = []
        cam_right_wrist = []
        ...
        last_state = None
        for j in range(0, right_gripper_all.shape[0]):
            ...
            # 如果模型期望 qpos = [left_arm, left_gripper, right_arm, right_gripper]
            # 请根据你下游模型的输入顺序调整！
            # 示例：假设顺序是 left + right
            state = np.concatenate([left_arm, left_gripper, right_arm, [right_gripper]], axis=0)
            state = state.astype(np.float32)

            if j != right_gripper_all.shape[0] - 1:

                qpos.append(state)

                camera_high_bits = image_dict["head_camera"][j]
                camera_high = decode_jpeg_bytes(camera_high_bits)
                # camera_high = cv2.imdecode(np.frombuffer(camera_high_bits, np.uint8), cv2.IMREAD_COLOR)
                camera_high_resized = cv2.resize(camera_high, (640, 480))   # TODO:
                cam_high.append(camera_high_resized)

                # === 虚拟手腕图像：生成纯黑图像 ===
                black_img = np.zeros((480, 640, 3), dtype=np.uint8)  # 黑色 640x480 BGR
                cam_right_wrist.append(black_img)
                cam_left_wrist.append(black_img)
...
if __name__ == "__main__":
    ...
    load_dir = os.path.join("/workspace/data_real", str(task_name), str(task_config), "data")
    ...
    for idx in range(expert_data_num):
        ...
        data_file_path = (f"/workspace/data_real/{task_name}/{task_config}/instructions/episode{idx}.json")
